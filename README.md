...
| [dsh-memory](dsh-memory/) | dsh-memory | 本地记忆：自动保存/回忆凭据、偏好、事实（仅存本机，绝不上传） |
...
# 3. 安装 dsh-mobile / dsh-memory（客户端/主机插件，需手动挂载）
cd ~/.dsh/profiles/web
pnpm add file:/绝对路径/dsh-plugins/dsh-mobile file:/绝对路径/dsh-plugins/dsh-memory
```
